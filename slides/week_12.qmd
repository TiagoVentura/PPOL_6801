...
**Definition:** a Neural Network with a specific structure that includes a
mechanism called self-attention. It has become the standard architecture for building large language models

**Publication:** first introduced at in the paper [Attention is All You Need](https://arxiv.org/pdf/1706.03762) by a group of researchers from Google Brain.

**Usage:** Core architecture behind most recent developments of Natural Language Processing (LLMs):
...
The development of Transformers comes from tasks based on next word prediction (as the Word Embeddings Algorithm we saw). These are at the foundation two core NLP applications: 
...
- **Training via masking:** Masked inputs to train bi-directional models
...
**Paper:** Y. Bengio, et al. (2000/2003): A Neural Probabilistic Language Model
...
- Nearby words are often more important becuase they are incorporated more recently
...
- **Masked Attention** Masked inputs to train bi-directional models
...
- Word2vec:  the representation of a word’s meaning is always the same vector irrespective of the contex
...
- The attention calculate occurs for every token $i$ with respects to all other tokens $j$ in the sequence. 
...
Modern large language models (LLMs) are all based on transformers. You can see how good they are!

There are encoder-only models (e.g., BERT), decoder-only models (e.g.,
