...
How can we systematically classify text into meaningful categories (e.g.,
positive/negative, hate vs. non-hate speech, spam vs. non-spam)?
...
- Differences: loss function they minimize, model assumptions, regularization
control (balance bias vs. variance), computational complexity
...
- Parameters that we set before training begins (e.g. learning rate,
number of neural networks layers, strength of regularization)
:::
...
You are working for the FBI, looking for emails that pertain to terrorist
attacks. Fortunately, such emails are very, very rare (0.0001% of all emails).

- For such tasks, there’s a trade-off between precision and recall.
Explain why.

-  We may be skeptical of using accuracy as a performance indicator
