...
-   Popular embedding method; fast to train, with pre-trained embeddings available online

-   Trained on large text corpora such as Google News (100 billion words), Wikipedia, and other massive datasets
...
::: fragment
$$P(w_t|w_{t-1}) = \frac{exp(u_c \cdot u_t)}{{\sum_{w}^V exp(u_c*u_w)}}$$ - mathematically, this is the logistic transformation (sigmoid) for multiple classes.
:::
...
**Negative Log-Likelihood + Decomposing the log**
...
::: incremental
-   Explain these three concepts: [Self-Supervision]{.red}, [Distributional Hypothesis]{.red}, [Dense Representation]{.red}. And put these three concepts together to explain how word vectors can capture meaning of words.
...
Consider: Window size, Number of dimensions for the embedding matrix, Pre-trained vs locally fit variants, which algorithm to use between Glove and Word2Vec.
...
1.  Compute Unigram Probabilities: Measure how often each word appears in the corpus

2.  Compute Word Co-occurrences: Count how often words appear together across the entire corpus
...
4.  Construct a Co-occurrence Matrix: Store PMI values in a large word-by-word matrix

5.  Apply Singular Value Decomposition (SVD): Reduce the matrix to extract meaningful lower-dimensional word vectors
